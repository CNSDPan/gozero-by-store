...
func (l *InitChatLogLogic) InitChatLog(in *api.InitChatLogReq) (res *api.InitChatLogRes, err error) {
	...
	items, e := l.svcCtx.MysqlQuery.StoreMember.InitChatLog(
		mysqlQuery.NewPage(10, 0),
		in.UserId,
	)
	if e != nil {
		...
		rows := make([]*api.StoreChatItem, len(items.GetRecords().([]mysqlModel.MemberChatLog)))
		for k, _ := range items.GetRecords().([]mysqlModel.MemberChatLog) {
			chat := items.GetRecords().([]mysqlModel.MemberChatLog)[k]
			parsedTimeStr := ""
			timestampStr := "0"
